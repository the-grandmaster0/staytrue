...
// Sends email notifications (+ stores in-app inbox) for:
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Helper: call send-email ──────────────────────────────────────────────
    const sendEmail = async (userId: string, title: string, body: string, url: string) => {
      try {
        const res = await fetch(`${supabaseUrl}/functions/v1/send-email`, {
          ...
          body: JSON.stringify({ user_id: userId, title, body, url, type: 'challenge', pref_key: 'challenges' }),
        });
        if (!res.ok) {
          const text = await res.text().catch(() => '');
          console.error(`[notify-challenge] send-email failed for ${userId}: ${res.status} ${text}`);
        }
        return res.status;
      } catch (err) {
        console.error(`[notify-challenge] send-email threw for ${userId}:`, err);
        return 0;
      }
    };
    ...
    if (eventType === 'INSERT' && status === 'pending') {
      ...
      if (winnerId === record.challenger_id) {
        ...
        const challengerName = await getName(record.challenger_id);
        await Promise.all([
          sendEmail(record.challenger_id, '🏆 You won!', `You beat ${opponentName} ${challengerScore}–${opponentScore} in the ${record.category} challenge!`, '/dashboard/challenges'),
          sendEmail(record.opponent_id, '💀 You lost!', `You lost ${opponentScore}–${challengerScore} to ${challengerName} in the ${record.category} challenge. Challenge them back!`, '/dashboard/challenges'),
        ]);
      } else if (winnerId === record.opponent_id) {
        const challengerName = await getName(record.challenger_id);
        const opponentName   = await getName(record.opponent_id);
        await Promise.all([
          sendEmail(record.opponent_id, '🏆 You won!', `You beat ${challengerName} ${opponentScore}–${challengerScore} in the ${record.category} challenge!`, '/dashboard/challenges'),
          sendEmail(record.challenger_id, '💀 You lost!', `You lost ${challengerScore}–${opponentScore} to ${opponentName} in the ${record.category} challenge. Get them next time!`, '/dashboard/challenges'),
        ]);
      } else {
        ...
        await Promise.all([
          sendEmail(record.challenger_id, '🤝 It\'s a draw!', `Your ${record.category} challenge vs ${opponentName} ended ${challengerScore}–${opponentScore}. Dead even!`, '/dashboard/challenges'),
          sendEmail(record.opponent_id, '🤝 It\'s a draw!', `Your ${record.category} challenge vs ${challengerName} ended ${opponentScore}–${challengerScore}. Dead even!`, '/dashboard/challenges'),
        ]);
      }
    }
